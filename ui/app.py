...
async def chat(q: Q):
    ...
    add_card(q, 'chat_card', ui.chatbot_card(box=ui.box('vertical', height='500px'), name='chatbot',
...
async def datasets(q: Q):
    ...
    clear_cards(q)  # When routing, drop all the cards except of the main ones (header, sidebar, meta).
    add_card(q, 'data_header', ui.form_card(
        box='horizontal',
        title='Dataset',
        items=[]
    ))
    ...
    add_card(q, 'table', ui.form_card(box='vertical', items=[ui.table(
        name='table',
        columns=[ui.table_column(name=i, label=i, sortable=True, data_type='string' if q.client.data[i].dtype == "O" else 'number') for i in q.client.data.columns],
        rows=[ui.table_row(name='row{}'.format(idx), cells=[str(i) for i in row]) for idx, row in q.client.data.iterrows()])
    ]))
...
async def init(q: Q) -> None:
    ...
    q.page['sidebar'] = ui.nav_card(
        box='sidebar', color='primary', title='QnA Assistant', subtitle="Get answers to your questions.",
    ...
    # Connect to LLM
    openai.api_key = app_args.get('llm_authorization')
